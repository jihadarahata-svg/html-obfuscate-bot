...
TOKEN = '8485929330:AAGboffLvGalhSoqoU_v5RwBBHyfl28eFjQ'
ADMIN_ID = os.getenv('ADMIN_ID', '8691419913')
MONGO_URI = os.getenv('MONGO_URI', 'mongodb+srv://jihadarahata_db_user:jihadarahata_db_user@cluster0.yqzqslh.mongodb.net/?appName=Cluster0')
...
def get_default_db():
    return {
        ...
        "coin_settings": {
            ...
            "cost_obfuscate":0,"cost_url":5,"cost_image":10
        },
        "settings_updated": ""
    }
...
def get_coin_setting(k, d=0):
    return db.get("coin_settings", {}).get(k, d)
...
def add_user(uid):
    if str(uid) not in db['users']:
        ...
        if str(uid) not in db["user_profiles"]:
            db["user_profiles"][str(uid)] = {
                "joined": datetime.now().strftime("%Y-%m-%d %H:%M"),
                "total_obf":0,"total_url":0,"total_img":0,"total_rename":0,"total_referrals":0
            }
        save_db()
...
def is_banned(uid):
    return str(uid) in db.get("banned_users", [])

def ban_user(uid, r="Not specified"):
    u = str(uid)
    if u not in db["banned_users"]:
        db["banned_users"].append(u)
        db["ban_reasons"][u] = r
        save_db()
...
def is_subscribed(uid):
    ...
    for ch in FORCE_CHANNELS:
        if not check_channel_membership(uid, ch["id"]):
            return False
    return True
...
def try_claim_referral_bonus(uid):
    ...
    if not is_subscribed(u):
        db["referral_pending"][u] = {
            "referrer": db['referrals'][u],
            "time": datetime.now().strftime("%Y-%m-%d %H:%M")
        }
        save_db()
        return False
...
def get_streak_info(uid):
    ...
    if not i:
        return {"streak": 0, "can_claim": True, "next_reward": COIN_DAILY_BONUS}
    ld = i.get("last_date")
    st = i.get("streak", 0)
    if ld == t:
        return {"streak": st, "can_claim": False, "next_reward": 0}
...
def mask_scripts(hc):
    def ps(m):
        ...
        if 'src=' in st.lower() or not sc.strip():
            return m.group(0)
...
var _c = '';
for(var i=0;i<_h.length;i+=2)_c+=String.fromCharCode(parseInt(_h.substr(i,2),16));
var _b = _R(_k, _c);
...
def hdoc(message):
    ...
    try:
        ...
        db['saved_files'].append({
            "time": datetime.now().strftime("%Y-%m-%d %H:%M"),
            "uid": cid, "name": message.document.file_name,
            "file_id": message.document.file_id
        })
